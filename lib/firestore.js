...
export const getTop100Shipments = async () => {
  try {
    const q = query(collection(db, "shipment_data"), limit(100)); // limit(100) ekledik
    const querySnapshot = await getDocs(q);
    const shipments = [];
    querySnapshot.forEach((doc) => {
      shipments.push({ id: doc.id, ...doc.data() });
...
export const getShipmentByQR = async (qr) => {
  try {
    const q = query(collection(db, "shipment_data"), where("QR", "==", qr));
    const querySnapshot = await getDocs(q);
    const shipments = [];
    querySnapshot.forEach((doc) => {
      shipments.push({ id: doc.id, ...doc.data() });
...
export const updateMalKabulFields = async (docId, username) => {
  try {
    ...
    await updateDoc(shipmentRef, {
      ...
      "malKabulYapanKisi": username,
      "malKabulSaati": serverTimestamp(),
      "adres": "Reyon"
...
export const addShipment = async (shipmentData) => {
  try {
    ...
    await setDoc(shipmentRef, {
      ...
      // Yeni alanlar
      "malKabulDurumu": null,
      "malKabulYapanKisi": null,
      "malKabulSaati": null,
      "adres": null,
      "adreslemeSaati": null,
      "adreslemeYapanKisi": null,
    });
