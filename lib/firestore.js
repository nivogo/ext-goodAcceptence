...
export const addMissingBox = async (box, paad_id, username) => {
  try {
    // Öncelikle authorization yenileniyor
    await refreshAuthorization();
    // Eklenecek yeni satır için bodyData oluşturuluyor.
    const bodyData = {
      data: {
        box: box,
        paad_id: paad_id,
        on_kabul_durumu: "3", // Yeni eklenen satır için ön kabul durumu 3
        on_kabul_yapan_kisi: username,
        on_kabul_saati: new Date().toISOString(),
        pre_accept_wh_id: paad_id,
        pre_accept_datetime: new Date().toISOString(),
        box_exist: false,
        created_at: new Date().toISOString() // İsteğe bağlı; oluşturulma zamanı
      }
    };
    // POST isteği ile yeni satır ekleniyor.
    const response = await fetch(API_BASE, {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
    ...
    const json = await response.json();
    console.log("addMissingBox: Yeni koli eklendi:", json);
    return json.shipmentId;
  } catch (error) {
