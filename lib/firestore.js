 ...
 * (PUT isteği ile "id: -1", "to_sap_location_id: -1" gönderilir.)
 */
...
export const getBoxesForBasariliKolilerByPreAccept = async (to_sap_location_id) => {
  try {
    await refreshAuthorization();
    const url = buildUrl({ pre_accept_wh_id: to_sap_location_id });
 ...
 */
export const getAllShipments = async (to_sap_location_id) => {
  try {
    ...
    const url = buildUrl({ to_sap_location_id });
...
/**
 * to_sap_location_id'ye göre gönderimleri çekme.
 * URL’ye ?to_sap_location_id=<to_sap_location_id> eklenir.
 */
export const getShipmentsByPAADID = async (to_sap_location_id) => {
  try {
    await refreshAuthorization();
    const url = buildUrl({ to_sap_location_id });
    const response = await fetch(url, {
...
/**
 * to_sap_location_id'ye göre başarılı kolileri çekme.
 * URL’ye ?to_sap_location_id=<to_sap_location_id> eklenir.
 */
export const getBoxesForBasariliKoliler = async (to_sap_location_id) => {
  try {
    await refreshAuthorization();
    const url = buildUrl({ to_sap_location_id });
    const response = await fetch(url, {
 ...
 * URL’ye ?to_sap_location_id=<to_sap_location_id> eklenir.
 */
export const getSuccessfulBoxes = async (to_sap_location_id) => {
  try {
    await refreshAuthorization();
    const url = buildUrl({ to_sap_location_id });
    const response = await fetch(url, {
 ...
 * URL’ye ?to_sap_location_id=<to_sap_location_id> eklenir.
 */
export const getBoxesWithOnKabulDurumu = async (to_sap_location_id) => {
  try {
    await refreshAuthorization();
    const url = buildUrl({ to_sap_location_id });
    const response = await fetch(url, {
 ...
 */
export const updateMalKabulFields = async (id, username, to_sap_location_id) => {
  try {
    await refreshAuthorization();
    const bodyData = {
      where: { id: id },
      data: {
        ...
        mal_kabul_saati: new Date(Date.now() + 3 * 60 * 60 * 1000).toISOString(),
        accept_wh_id: to_sap_location_id,
        accept_datetime: new Date(Date.now() + 3 * 60 * 60 * 1000).toISOString(),
        adres: "REYON",
        adresleme_saati: new Date(Date.now() + 3 * 60 * 60 * 1000).toISOString(),
 ...
 */
export const updateOnKabulFields = async (id, username, to_sap_location_id) => {
  try {
    await refreshAuthorization();
    console.log("updateOnKabulFields: to_sap_location_id gönderiliyor:", to_sap_location_id);
    const bodyData = {
      where: { id: id },
      data: {
        on_kabul_durumu: 1,
        on_kabul_yapan_kisi: username,
        on_kabul_saati: new Date(Date.now() + 3 * 60 * 60 * 1000).toISOString(),
        pre_accept_wh_id: to_sap_location_id,
        pre_accept_datetime: new Date(Date.now() + 3 * 60 * 60 * 1000).toISOString(),
        adres: "DEPO",
        adresleme_saati: new Date(Date.now() + 3 * 60 * 60 * 1000).toISOString(),
        adresleme_yapan_kisi: username
      },
 ...
 */
export const markExtraBox = async (id, username, to_sap_location_id) => {
  try {
    await refreshAuthorization();
    console.log("markExtraBox: to_sap_location_id gönderiliyor:", to_sap_location_id);
    const bodyData = {
      where: { id },
      data: {
...
export const updateQRForDifferent = async (id, username, to_sap_location_id) => {
  try {
    await refreshAuthorization();
    console.log("updateQRForDifferent: to_sap_location_id gönderiliyor:", to_sap_location_id);
    const bodyData = {
      where: { id },
      data: {
        ...
        mal_kabul_yapan_kisi: username,
        mal_kabul_saati: new Date(Date.now() + 3 * 60 * 60 * 1000).toISOString(),
        accept_wh_id: to_sap_location_id, // Kullanıcının to_sap_location_id'si
        accept_datetime: new Date(Date.now() + 3 * 60 * 60 * 1000).toISOString(),
        adres: "REYON",
        adresleme_yapan_kisi: username,
 ...
 * Body: { data: { to_sap_location_id, qr, created_at } }
 */
...
export const addMissingBox = async (box, to_sap_location_id, username) => {
  try {
    await refreshAuthorization();
    ...
    const shipmentData = {
        box: box,
        to_sap_location_id: to_sap_location_id,
        on_kabul_durumu: 3, // 3: Eksik koli için işaretleme
        on_kabul_yapan_kisi: username,
        on_kabul_saati: new Date(Date.now() + 3 * 60 * 60 * 1000).toISOString(),
        pre_accept_wh_id: to_sap_location_id,         // pre_accept_wh_id, okutmayı yapanın to_sap_location_id'si
        pre_accept_datetime: new Date(Date.now() + 3 * 60 * 60 * 1000).toISOString(),
...
export const addMissingQR = async (qr, box, to_sap_location_id, username) => {
  try {
    await refreshAuthorization();
