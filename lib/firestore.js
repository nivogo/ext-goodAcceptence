...
/**
 * URL oluşturucu: Eğer query string boşsa soru işareti eklemeyin.
 */
...
/**
 * onKabul_durumu dolu kolileri çekme.
 * URL’ye ?paad_id=<paad_id> eklenir.
 */
 ...
 * Body: { where: { id: <docId> }, data: { mal_Kabul_durumu, mal_kabul_yapan_kisi, mal_kabul_saati } }
 */
 ...
 * Body: { where: { id: <docId> }, data: { on_kabul_yapan_kisi, on_kabul_saati } }
 */
 ...
 * Body: { where: { id: <docId> }, data: { extraBoxMarkedBy, extraBoxMarkedAt } }
 */
export const markExtraBox = async (docId, username) => {
  try {
    ...
    const bodyData = {
      where: { id: docId },
      data: {
        extraBoxMarkedBy: username,
        extraBoxMarkedAt: new Date().toISOString(),
      },
    };
    const response = await fetch(API_BASE, {
      method: "PUT",
...
/**
 * Adres güncellemesi.
 * Body: { where: { id: <docId> }, data: { adres, adresleme_yapan_kisi, adresleme_saati } }
 */
